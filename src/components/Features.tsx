...
const ICONS: Record<IconName, ReactElement> = {
    ...
    bolt: icon(<path d="M13 2 3 14h7l-1 8 10-12h-7l1-8z" />),
    layers: icon(
        <>
    ...
    camera: icon(
        <>
            <path d="M3 8a2 2 0 0 1 2-2h2l2-3h6l2 3h2a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8z" />
...
export default function Features() {
    return (
        <Section id="features" className="features">
            <h2>Everything a reading habit needs</h2>
            <p className="section-sub">
                One app for the books you read and the words they teach you.
            </p>
            <div className="features-grid">
                {FEATURES.map(({ icon: name, title, description }) => (
                    <article key={title} className="feature-card">
                        <span className="feature-icon">{ICONS[name]}</span>
                        <h3>{title}</h3>
                        <p>{description}</p>
                    </article>
                ))}
